...
import { LogEntry, LogMetadata, ErrorMetadata } from '../../shared/types'
...
export class Logger {
  ...
  public static init(logDir: string = DEFAULT_LOG_DIR): void {
    ...
    try {
      ...
      try {
        ...
        const e: unknown = err as unknown
        // 文件不存在是正常情况，无需处理
  ...
  public static error(
    operation: string,
    message: string,
    error: unknown = null, // 接受一个 Error 对象或任意未知类型
    tableName: string | null = null,
    entityId: string | null = null
  ): void {
    // 尝试提取 Error 对象的堆栈信息作为 metadata
    const metadata: ErrorMetadata | LogMetadata =
      error instanceof Error
        ? { name: error.name, stack: error.stack, error: error.message }
        : (error as LogMetadata)
