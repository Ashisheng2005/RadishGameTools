...
export function setupAutoStart(autoLaunchAtStartup: boolean) {
    ...
    Logger.info("auto start", `Setting up auto-start: ${autoLaunchAtStartup}, isElevated: ${isElevated}`)
    ...
    if (appPath.includes(' ') || appPath.includes('[') || appPath.includes(']')) {
        ...
        if (process.platform === 'win32') {
            try {
                ...
                const { execSync } = require('child_process')
                const shortPath = execSync(`cmd /c for %A in ("${appPath}") do @echo %~sA`).toString().trim()
