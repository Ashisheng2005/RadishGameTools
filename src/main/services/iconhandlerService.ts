...
// 读取文件返回base64
export async function readIconFileAsBase64(filePath: string): Promise< {success: boolean; base64Data?: string; mimeType?: string}> {
  try {
...
export async function extractIconFromExe(executablePath: string) {
  try {
    ...
    // 将base64图标保存为文件
    const result = await saveIconFile({ base64Data: base64Icon, appId: path.basename(executablePath, path.extname(executablePath)) })
    if (!result.success) {
...
try {
    ...
    // console.log(`${escapedExePath} ${escapedOutputPath}`)
    const { stdout, stderr } = await execAsync(`powershell -ExecutionPolicy Bypass -File "${tempScriptPath}" "${escapedExePath}" "${escapedOutputPath}"`)
  ...
  try {
    ...
    if (iconPath) {
      ...
      if (!icon.isEmpty()) {
        ...
        // 保存文件
        const result = await saveIconFile({ base64Data: base64Icon, appId: path.basename(executablePath, path.extname(executablePath)) + '_system' })
        if (result.success) {
          return {
            found: true,
            iconPath: result.savedPath,
