...
export class DataService {
  ...
  async recordAppEnd(sessionId: string, appId: string, durationFromRunner?: number): Promise<Boolean> {
      ...
      if (durationFromRunner !== undefined) {
        try {
          ...
          if (!updated) {
            ...
            Logger.info('dataSqlService-recordAppEnd', `Inserted fallback completed session ${sessionId} for app ${appId}`)
    ...
    if (session.startTime) {
      ...
      Logger.info('dataSqlService-recordAppEnd', `recordAppEnd session.startTime: ${session.startTime} endTimeDate: ${endTimeDate}`)
  ...
  async updateAppStats(appId: string, duration: number): Promise<void> {
    try {
      Logger.info('dataSqlService-updateAppStats', `update app total info: ${appId}, running duration: ${duration} s`)
  ...
  async getAppHealthStatus(appId: string): Promise<{
    ...
    const crashRate = sessions.length > 0 
        ? (crashedSessionsCount / sessions.length) * 100 
        : 0;
