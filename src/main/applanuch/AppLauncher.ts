...
export class AppLauncher extends EventEmitter {
  ...
  private async _launchApp(
    app: AppData
  ): Promise<{ success: boolean; error?: string; sessionId?: string }> {
    try {
      Logger.info(
        'AppLaunch-launchApp',
        `satrt appp.name: ${app.id}, app.path: ${app.executablePath}`
      )
    ...
    try {
      ...
      if (runningApp.sessionId) {
        ...
        if (!success) {
          Logger.warn(
            'AppLaunch-launchApp',
            `Session ${runningApp.sessionId} not found or already ended.`
          )
        }
      }
      ...
      Logger.error(
        'AppLaunch-lhandleProcessExit',
        `Error handling process exit for app ${appId}:`,
        error
      )
    }
  }
  ...
  private _terminateApp(appId: string, force?: boolean): boolean {
    ...
    try {
      const safeProc = proc as ChildProcess & { killed?: boolean; exitCode?: number | null }
      if (safeProc.killed || safeProc.exitCode != null) {
    ...
    try {
      if (force) {
        ...
        // 在超时后尝试强制终止（防止进程无响应）
        const safeProc = proc as ChildProcess & {
          killed?: boolean
          exitCode?: number | null
          pid?: number
        }
        const pid = safeProc.pid
        setTimeout(() => {
          try {
            if (pid && !safeProc.killed && safeProc.exitCode == null) {
              try {
                safeProc.kill('SIGKILL')
                ...
                try {
                  ...
                  Logger.error(
                    'AppLauncher_terminateApp',
                    `force kill fallback failed for ${appId}`,
                    taskErr
                  )
                }
              }
            ...
            Logger.error(
              'AppLauncher_terminateApp',
              `timeout force-kill check failed for ${appId}`,
              inner
            )
          }
        }, 3000)
      ...
      try {
        const safeProc = proc as ChildProcess & { pid?: number }
        const pid = safeProc.pid
        if (process.platform === 'win32' && pid) {
          execSync(`taskkill /PID ${pid} /T /F`)
          this._activeModification()
        ...
        Logger.error(
          'AppLauncher_terminateApp',
          `fallback taskkill failed for ${appId}`,
          fallbackErr
        )
      }
  ...
  private _getAppStatus(appId: string): { status: string; duration: number; pid?: number } {
    ...
    const duration = Math.floor((new Date().getTime() - runningApp.startTime.getTime()) / 1000)
  ...
  // 获取所有运行中的应用
  static getAllRunningApps(): Array<{
    appId: string
    startTime: Date
    duration: number
    pid?: number
  ...
  private _getAllRunningApps(): Array<{
    appId: string
    startTime: Date
    duration: number
    pid?: number
