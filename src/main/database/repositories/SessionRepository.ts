    ...
    try {
      ...
      if (check.count > 0) {
        ...
        Logger.info(
          ...
          `Immediate verification failed: Session ${session.id} not found after insertion. DB size: ${dbSize} bytes`
        )
      ...
      Logger.error(
        'Sessionrepository-addSession',
        `Failed to add session ${session.id} for app ${appId}:`,
        error
      )
  ...
  public async getSessionById(id: string): Promise<Session | null> {
    ...
    if (row) {
      ...
      Logger.warn(
        ...
        `getSessionById: session ${id} NOT FOUND. DB size: ${dbSize} bytes`
      )
    ...
    try {
      ...
      if (!existingSession) {
        Logger.error(
          ...
          `Session ID ${id} not found for status update.`
        )
      ...
      if (result.changes > 0) {
        ...
        Logger.error(
          'SessionRepository-updateSessionStatus',
          `Failed to update session ${id}: No changes made.`
        )
