...
// DatabaseService 是整个应用程序数据库操作的单例入口, 它封装了 Repository 层的逻辑，提供高层接口并管理数据库度量。
export class DatabaseService {
  ...
  public async getCategoryStats(): Promise<
    { category: string; totalDuration: number; appCount: number }[]
  > {
    this.metrics.queryCount++
  ...
  private async updateMetrics(): Promise<void> {
    try {
      ...
      Logger.error('databasesql-updateMetrics', 'Failed to update metrics:', error)
      //  this.logger.log({
      //     level: 'error',
