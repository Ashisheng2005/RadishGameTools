...
export default function LauncherPage() {
    ...
    if (isUpdate.state) {
      ...
      try {
        ...
        setActiveApps((prev) => {
          ...
          runningApps.forEach((app: unknown) => {
            if (!app || typeof app !== 'object') return
            const maybe = app as Record<string, unknown>
            const appId =
              (typeof maybe.appId === 'string' && maybe.appId) ||
              (typeof maybe.id === 'string' && maybe.id) ||
              undefined
            if (appId) {
              const rawStart = maybe.startTime
              const startTime =
          ...
          if (prev.size !== next.size || Array.from(prev.keys()).some((key) => !next.has(key))) {
  ...
  const loadAllApps = async () => {
    try {
      ...
      await window.electronAPI.loggerInfo(
        'page',
        `Loading ${appData.length} applicaitons successful`
      )
    } catch (error) {
  ...
  const handleAddApp = useCallback(async (newAppData: NewAppData) => {
    try {
      ...
      await window.electronAPI.loggerInfo(
        'append applicaiton',
        `app name is  '${newAppData.name}' , ID: ${appId}, icon path: ${newAppData.icon}`
      )
    } catch (error) {
  ...
  const handleLaunchAppKernel = async (app: AppData) => {
    ...
    if (result.success) {
      await window.electronAPI.loggerInfo(
        'run',
        `run applicaiton appid: ${app.id} app_path: ${app.executablePath} successful startup`
      )
      ...
      // 更新最后使用时间
      setApps((prevApps) =>
        prevApps.map((a) => (a.id === app.id ? { ...a, lastUsed: new Date().toISOString() } : a))
      )
      ...
      await window.electronAPI.loggerError(
        'run',
        `run applicaiton appid: ${app.id} app_path: ${app.executablePath} failed startup, ${result.error}`
      )
  ...
  // 停止函数核心， 同样的，只是核心
  const handleStopAppKernel = useCallback(
    async (app: AppData) => {
