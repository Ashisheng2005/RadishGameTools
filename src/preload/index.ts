...
const customElectronAPI = {
  ...
  // 提取缩略图到Buffer
  extraceThumbnail: (filePath: string) => 
    ipcRenderer.invoke('extrace-thumbnail', filePath),
  ...
  // 添加获取文件路径方法
  getFilePath: (file: File) => (file as any).path || null,

  // 获取Modification状态
  getModification: () =>
    ipcRenderer.invoke('app:getModification'),
  ...
  getAppStatus: (appId: string) => 
    ipcRenderer.invoke('app:getStatus', appId),

  // 获取所有正在运行的应用（来自主进程的 AppLauncher）
  getAllRunningApps: () =>
    ipcRenderer.invoke('app:getAllRunning'),


  getAllApps: () => 
    ipcRenderer.invoke('get-all-apps'),


  getApp: (appId: string) => 
    ipcRenderer.invoke('get-app', appId),
  ...
  getAppStart: (appId: string) =>
    ipcRenderer.invoke('get-app-start', appId),

  saveApp: (app: AppData) =>
    ipcRenderer.invoke('save-app', app),

  deleteApp: (appId: string) =>
    ipcRenderer.invoke('delete-app', appId),

  loggerInfo: (operation: string, message: string, metadata: any = null, tableName: string | null = null, entityId: string | null = null) => 
    ipcRenderer.invoke('log-message', 'info', operation, message, metadata, tableName, entityId),
  ...
  // 获取窗口状态
  getWindowState: () =>
    ipcRenderer.invoke('window-state:get'),
